...
export default function useChartHook() {
  ...
  useEffect(() => {
    if (active.length > 0 && data[active.at(-1)]) {
      if (YEAR) {
        const dataYear = Object.values(data[active.at(-1)][dataType][obsDepth].year)
        ...
        console.log("02", { dataYearActualized })
  ...
  const setDataLines = useCallback(
    ...
    [chart, charType, dataType, obsTypes, unit, YEAR, lakesChartOptions, yearsChartOptions]
  )
